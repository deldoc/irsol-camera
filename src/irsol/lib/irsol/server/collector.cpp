...
void
FrameCollector::refreshFrameRate()
{
  ...
  for(double r : frameRates) {
    ...
    if(std::abs(ratio - rounded) > eps) {
      IRSOL_LOG_WARN(
        "Client rate {:.2f} fps is not an integer divisor of max fps {:.2f}; "
...
bool
FrameCollector::hasClient(std::shared_ptr<ClientSession> client) const
{
  std::lock_guard<std::mutex> lock(m_clientsMutex);
  return std::any_of(m_clients.begin(), m_clients.end(), [client](const auto& pair) {
    return pair.first == client;
  });
}

void
FrameCollector::addClient(std::shared_ptr<ClientSession> client, CollectedFrameCallback callback)
{
  ...
  }
  {
    ...
    IRSOL_LOG_INFO(
      "Client {} added to frame collector, total clients: {}", client->id(), m_clients.size());
  }
...
void
FrameCollector::removeClient(std::shared_ptr<ClientSession> client)
{
  {
    std::lock_guard<std::mutex> lock(m_clientsMutex);
    auto it = std::find_if(m_clients.begin(), m_clients.end(), [client](const auto& pair) {
      return pair.first == client;
    });
    if(it != m_clients.end()) {
      m_clients.erase(it);
      IRSOL_LOG_INFO(
        "Client {} removed from frame collector, total clients: {}",
        client->id(),
        m_clients.size());
      m_frameAvailableCond.notify_all();
    } else {
...
void
FrameCollector::broadcastFrames()
{
  ...
  while(true) {
    ...
    m_frameAvailableCond.wait(frameAvailableCondLock, [this] {
      ...
      IRSOL_LOG_TRACE(
        "Frame broadcaster thread is awakened for {} clients, and {} frames.",
        numClients,
        numFrames);
      return true;
    });
